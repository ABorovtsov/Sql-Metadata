...
class MetaProvider:
    def get(self, root_script_directory, include_scripts = None, table_aliases = None, overrides = {}):
        sql_paths = self.__get_script_paths(root_script_directory, include_scripts)
        return [SQLMetadata(path, table_aliases, overrides.get(self.__get_name(path), {})) for path in sql_paths]
